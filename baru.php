...
if (isset($_POST['update'])) {
	...
	if (empty($nama) || empty($qty) || empty($email)) {
		...
		if (!empty($filename)) {
			...
			$result = mysqli_query($mysqli, "UPDATE users SET nama='$nama',qty='$qty',email='$email',gambar='$filename' WHERE id=$id");
		}

		// MEMASUKAN DATA YANG DI UPDATE KECUALI GAMBAR
		$result = mysqli_query($mysqli, "UPDATE users SET nama='$nama',qty='$qty',email='$email' WHERE id=$id");
	...
	<center>
		...
		<form name="form1" method="post" action="edit.php" enctype="multipart/form-data">
			<table border="0">
				...
				<tr>
					<td>qty</td>
					<td><input type="text" name="qty" value="<?php echo $age; ?>"></td>
				</tr>
				<tr>
					<td>Email</td>
